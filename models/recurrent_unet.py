# develop reccurent attentation based squeeze Unet - its final

"""
New model : reccurent + Attenation + SqueezeUnet
Target dataset: Drone dataset
"""
...
def UNet(n_filters=16, bn=True, dilation_rate=1,batch_size=5,classes= 24):
    ...
    # inlcude squeeze_conv1_1 - see if concatenating all three works else for with concat1_2 and squeeze_conv1_1
    # if you want add squeeze_conv1_1
    Dense_1 =  concatenate([concat1_1, concat1_2, squeeze_conv1_1], axis=-1)
    ...
    # inlcude squeeze_conv2_1
    Dense_3 = concatenate([concat2_1, concat2_2, squeeze_conv2_1], axis=-1)
    ...
    expand_conv1x1_6 = Conv2D(256, (1, 1), strides=(1, 1), padding='same', dilation_rate=dilation_rate)(squeeze_conv2_2)
    expand_conv3x3_6 = Conv2D(256, (3, 3), strides=(1, 1), padding='same', dilation_rate=dilation_rate)(squeeze_conv2_2)
    concat3_2 = concatenate([expand_conv1x1_6, expand_conv3x3_6], axis=-1)
    Dense_5 = concatenate([concat3_1, concat3_2, squeeze_conv3_1], axis=-1)
    #Dense_1 = Activation('relu')

    # mid - Block 4
    squeeze_conv3_1 = Conv2D(80, (1, 1), strides=(2, 2), activation='relu', padding='same', dilation_rate=dilation_rate)(Dense_5)
    expand_conv1x1_7 = Conv2D(512, (1, 1), strides=(1, 1), padding='same', dilation_rate=dilation_rate)(squeeze_conv3_1)
    expand_conv3x3_7 = Conv2D(512, (3, 3), strides=(1, 1), padding='same', dilation_rate=dilation_rate)(squeeze_conv3_1)
    concat4_1 = concatenate([expand_conv1x1_7 ,expand_conv3x3_7], axis=-1)
    squeeze_conv2_2 = Conv2D(80, (1, 1), strides=(1, 1), activation='relu', padding='same', dilation_rate=dilation_rate)(concat4_1)
    expand_conv1x1_8 = Conv2D(512, (1, 1), strides=(1, 1), padding='same', dilation_rate=dilation_rate)(squeeze_conv2_2)
    expand_conv3x3_8 = Conv2D(512, (3, 3), strides=(1, 1), padding='same', dilation_rate=dilation_rate)(squeeze_conv2_2)
    concat4_2 = concatenate([expand_conv1x1_8, expand_conv3x3_8], axis=-1)
    Dense_6 = concatenate([concat4_1, concat4_2, squeeze_conv3_1], axis=-1)
    ...
    upconcat0_2 = concatenate([expand_upconv1x1_2, expand_upconv3x3_2, s1], axis=-1)
    ...
    upconcat1_2 = concatenate([expand_upconv1x1_4, expand_upconv3x3_4, s2], axis=-1)
    ...
    upconcat2_2 = concatenate([expand_upconv1x1_6, expand_upconv3x3_6, s3], axis=-1)
    #upconcat1_2 = Activation('relu')

    end_concatenate = concatenate([upconcat2_2,squeeze_conv1_1], axis=-1)

    conv20 = Conv2DTranspose(64, (2, 2), strides=(2, 2), activation='relu', padding='same', dilation_rate=dilation_rate)(end_concatenate)

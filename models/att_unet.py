from tensorflow.keras.layers import Input, Add, Dropout, Permute, add, concatenate, UpSampling2D,Activation
from tensorflow.keras.layers import Convolution2D, ZeroPadding2D, MaxPooling2D, Cropping2D, Conv2D, BatchNormalization,multiply
...
def UNet(n_filters=16, bn=True, dilation_rate=1):
    ...
    attention_s5 = attention_block_2d(up5,conv4, n_filters * 16)
    up5 = concatenate([attention_s5, up5 ], axis=3)

    conv6 = Conv2D(n_filters * 8, (3, 3), activation='relu', padding='same', dilation_rate=dilation_rate)(up5)
    ...
    attention_s6 = attention_block_2d(up6, conv3, n_filters * 8)
    up6 = concatenate([attention_s6, up6], axis=3)


    conv7 = Conv2D(n_filters * 4, (3, 3), activation='relu', padding='same', dilation_rate=dilation_rate)(up6)
    if bn:
    ...
    attention_s7 = attention_block_2d(up7, conv2, n_filters * 4)
    up7 = concatenate([attention_s7, up7], axis=3)

    #up8 = concatenate([UpSampling2D(size=(2, 2))(conv7), conv2], axis=3)

    conv8 = Conv2D(n_filters * 2, (3, 3), activation='relu', padding='same', dilation_rate=dilation_rate)(up7)
    if bn:
    ...
    attention_s8 = attention_block_2d(up8, conv1, n_filters * 2)
    up8 = concatenate([attention_s8, up8], axis=3)

    #up9 = concatenate([UpSampling2D(size=(2, 2))(conv8), conv1], axis=3)

    conv9 = Conv2D(n_filters * 1, (3, 3), activation='relu', padding='same', dilation_rate=dilation_rate)(up8)
    if bn:
